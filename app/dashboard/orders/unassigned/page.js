...
const OrdersPage = () => {
  ...
  const handleExport = async (format) => {
    try {
      setIsExporting(true);
      const exportFilters = { ...filters, hasRider: false };
      ...
      if (format === 'excel') {
        exportToExcel(allOrders, columns, 'unassigned-orders');
        toast({ title: "Success", description: "Unassigned orders exported to Excel successfully!" });
      } else if (format === 'pdf') {
        await exportToPDF(allOrders, columns, 'Unassigned Orders Report', 'unassigned-orders');
        toast({ title: "Success", description: "Unassigned orders exported to PDF successfully!" });
      }
    } catch (error) {
